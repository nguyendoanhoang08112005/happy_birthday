...
export default function GreetingCard() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim() || !wish.trim()) {
      setStatus("error");
      setErrorMessage("Vui lòng điền đầy đủ cả tên và điều ước của bạn nhé! 💕");
      return;
    }
      ...
      {status === "success" ? (
        ...
        <div className="bg-[#fffdf6] border-4 border-[#3a040e] rounded-[2rem] p-8 shadow-[8px_8px_0px_#3a040e] relative text-center flex flex-col items-center justify-center min-h-[340px] transform transition-all duration-500 scale-100">
          ...
          <button
            ...
            className="mt-6 px-6 py-2 bg-gradient-to-r from-amber-400 to-rose-500 text-white font-sans font-bold text-xs rounded-full shadow-md hover:scale-105 active:scale-95 transition-transform"
          >
            Viết điều ước khác
          </button>
        </div>
        ...
        >
          ...
          {/* Red/Gold Wax Seal Stamp decoration in bottom right corner */}
          <div className="absolute bottom-6 right-6 w-14 h-14 bg-gradient-to-br from-red-600 to-[#990011] rounded-full border-2 border-[#3a040e] shadow-md flex items-center justify-center transform rotate-12 opacity-80 pointer-events-none">
            <div className="w-10 h-10 border border-dashed border-yellow-300/60 rounded-full flex items-center justify-center">
          ...
          <div className="mb-6 relative">
            ...
            <textarea
              ...
              placeholder="Mong ước tuổi mới của bạn là gì? Hoặc gửi một lời nhắn ấm áp nhé..."
